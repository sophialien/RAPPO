...
from ._auto_LiRPA.bound_general import BoundedModule, BoundedTensor, BoundedParameter
from ._auto_LiRPA.perturbations import *
...
if forward_one:
    def get_kl_bound(model, x, means, eps, beta=None, stdev=None, use_full_backward=False):
        ...
        x.ptb = PerturbationLpNorm(norm=np.inf, eps=eps)
        if forward_one:
            inputs = (x, )
        else:
            inputs = (x, means)
