from policy_gradients.agent_Q import Trainer
import git
import pickle
import random
import numpy as np
...
def main(params):
    ...
    base_directory = params['out_dir'] + '_metaLR_'+ str(params['meta_lr'])+ '_w2_'+ str(params['second_w']) +'_w3_'+ str(params['third_w']) + '/adv_{}-ball+reg_{}+{}-ratio_{}-lambda_{}-'.format(params['adv_delta'],params['robust_ppo_eps'],params['robust_ppo_reg'],params['adv_delta_ball'],params['lambda']) + str(params['seed'])
    ...
    def finalize_table(iteration, terminated_early, rewards, adv_delta=0):
    ...
    try:
        for i in range(params['train_steps']):
            ...
            if params['save_iters'] > 0 and i % params['save_iters'] == 0 and i != 0:
                #print(rewards)
                final_5_rewards = np.array(rewards)[-5:].mean()
                adv_delta = np.array(adv_deltas)[-1].mean()
            ...
            mean_reward, plot_info = p.train_step()
            rewards.append(plot_info['mean_reward'])
            if 'adv_delta' in plot_info.keys():
                adv_deltas.append(plot_info['adv_delta'])
            else:
                adv_deltas.append(0)
        ...
        finalize_table(i, False, rewards, adv_deltas[-1])
    except KeyboardInterrupt:
        finalize_table(i, True, rewards, adv_delta[-1])
        ret = 1
    except:
        print("An error occurred during training:")
        traceback.print_exc()
        # Other errors, make sure to finalize the cox store before exiting.
        finalize_table(i, True, rewards, adv_delta)
        ret = -1
...
def add_common_parser_opts(parser):
    ...
    parser.add_argument('--test_attack_freq', type=int, default=20)
    parser.add_argument('--attack_transition', type=str2bool, const=True, nargs='?')
    ...
    parser.add_argument('--adv_delta', type=float, help='discount on reward')
    parser.add_argument('--adv_delta_ball', type=float, help='discount on reward')
  ...
  # PPO parameters
    ...
    parser.add_argument('--attack_from_danger', type=str2bool, help='collect perturbed states during training')
    ...
    # env parameter
    parser.add_argument('--mass-ratio', type=float, default=1.0)
    parser.add_argument('--friction-ratio', type=float, default=1.0)
    parser.add_argument('--gravity-ratio', type=float, default=1.0)
    parser.add_argument('--wind-ratio', type=float, default=1.0)
    
    # meta-gradient
    parser.add_argument('--meta-lr', type=float, default=1.0)
    parser.add_argument('--second-w', type=float, default=1.0)
    parser.add_argument('--third-w', type=float, default=1.0)
